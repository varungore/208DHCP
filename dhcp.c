...
#define DHCP_SERVER_PORT   6724
#define DHCP_CLIENT_PORT   6824

#define ETHERNET_HARDWARE_ADDRESS            1     /* used in htype field of dhcp packet */
#define ETHERNET_HARDWARE_ADDRESS_LENGTH     6     /* length of Ethernet hardware addresses */

unsigned char client_hardware_address[MAX_DHCP_CHADDR_LENGTH]="95524893075732";
unsigned int my_client_mac[MAX_DHCP_CHADDR_LENGTH] = {9,5,5,2,4,8,9,3,0,7,5,7,3,2};
int mymac = 0;
...
int main(int argc, char **argv){
	...
	get_dhcp_ack(dhcp_socket);
	//send_dhcp_request(dhcp_socket);
...
int send_dhcp_discover(int sock){
	...
	printf("client_hardware_address");
	for(i=0; i<16;i++)
	{
		printf("%d",client_hardware_address[i]);
	}
	printf("\n");
...
int send_dhcp_request(int sock){
...
struct sockaddr_in sources;
	...
	for(responses=0,valid_responses=0;;){
		...
		if(result!=OK){
			if (verbose)
				printf("Result: There is no Multiple from the servers...\n");
	continue;
		        }
		else{
			if (verbose) {
				printf("Result=Respose received from the server...\n");
	...
	memcpy (&dhcp_request_packet, &offer_packet, sizeof (dhcp_request_packet));
		if (verbose) {
...
if(OFFER)
{
printf(" Prepare DHCP Request\n");

//printf("%s\n",srvipaddr);
dhcp_request_packet.op=BOOTREQUEST;

	/* hardware address type */
	dhcp_request_packet.htype=ETHERNET_HARDWARE_ADDRESS;

	/* length of our hardware address */
	dhcp_request_packet.hlen=ETHERNET_HARDWARE_ADDRESS_LENGTH;

	dhcp_request_packet.hops=0;
	dhcp_request_packet.secs=0xFF;

	/* tell server it should broadcast its response */ 
	dhcp_request_packet.flags=htons(DHCP_BROADCAST_FLAG);
dhcp_request_packet.options[0]='\x63';
	dhcp_request_packet.options[1]='\x82';
	dhcp_request_packet.options[2]='\x53';
	dhcp_request_packet.options[3]='\x63';

	/* DHCP message type is embedded in options field */
	dhcp_request_packet.options[4]=DHCP_OPTION_MESSAGE_TYPE;    /* DHCP message type option identifier */
	dhcp_request_packet.options[5]='\x01';               /* DHCP message option length in bytes */
	dhcp_request_packet.options[6]=DHCPREQUEST;
if(request_specific_address==TRUE){
		dhcp_request_packet.options[7]=DHCP_OPTION_REQUESTED_ADDRESS;
		dhcp_request_packet.options[8]='\x04';
		memcpy(&dhcp_request_packet.options[9],&requested_address,sizeof(requested_address));
	        }
sources.sin_family=AF_INET;
        sources.sin_port=htons(DHCP_SERVER_PORT);
        sources.sin_addr.s_addr=INADDR_BROADCAST;
	bzero(&sources.sin_zero,sizeof(sources.sin_zero));

	if (verbose) {
		printf("DHCPREQUEST to %s port %d\n",inet_ntoa(sources.sin_addr),ntohs(sources.sin_port));
		printf("DHCPREQUEST XID: %lu (0x%X)\n",(unsigned long) ntohl(dhcp_request_packet.xid),ntohl(dhcp_request_packet.xid));
		printf("DHCPREQUEST ciaddr:  %s\n",inet_ntoa(dhcp_request_packet.ciaddr));
		printf("DHCPREQUEST yiaddr:  %s\n",inet_ntoa(dhcp_request_packet.yiaddr));
		printf("DHCPREQUEST siaddr:  %s\n",inet_ntoa(dhcp_request_packet.siaddr));
		printf("DHCPREQUEST giaddr:  %s\n",inet_ntoa(dhcp_request_packet.giaddr));
	}
send_dhcp_packet(&dhcp_request_packet,sizeof(dhcp_request_packet),sock,&sources);
...
char *srvipaddr;
	...
	for(responses=0,valid_responses=0;;){
		...
		if(result!=OK){
			if (verbose)
				printf("Result=ERROR ,DHCPACK\n");
	continue;
		        }
		else{
			if (verbose) {
				printf("Result=TTTTTTTT\n");

			responses = responses + 1;}
		        }

		if (verbose) {
			printf("DHCPACK from IP address %s\n",inet_ntoa(source.sin_addr));
			printf("DHCPACK XID: %lu (0x%X)\n",(unsigned long) ntohl(offer_packet.xid),ntohl(offer_packet.xid));
srvipaddr=inet_ntoa(source.sin_addr);
printf("%s\n",srvipaddr);
		...
		if(ntohl(offer_packet.xid)!=packet_xid){
			if (verbose)
				printf("DHCPACK XID (%lu) did not match DHCPDISCOVER XID (%lu) - ignoring packet\n",(unsigned long) ntohl(offer_packet.xid),(unsigned long) packet_xid);
		...
		if(result==ERROR){
			if (verbose) 
				printf("DHCPACK hardware address did not match our own - ignoring packet\n");

			continue;
		        }
	memcpy (&dhcp_request_packet, &offer_packet, sizeof (dhcp_request_packet));
		if (verbose) {
			printf("DHCPACK ciaddr: %s\n",inet_ntoa(offer_packet.ciaddr));
			printf("DHCPACK yiaddr: %s\n",inet_ntoa(offer_packet.yiaddr));
			printf("DHCPACK siaddr: %s\n",inet_ntoa(offer_packet.siaddr));
			printf("DHCPACK giaddr: %s\n",inet_ntoa(offer_packet.giaddr));
